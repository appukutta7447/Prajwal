...
function populateStrandSelectOption() {
    ...
    $stmt = $con->prepare('SELECT strand_id, strand_name FROM strands ORDER BY strand_name');
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($id, $strandName);

    while ($row = $stmt->fetch()) {
        ?>
            <option value="<?php echo encryptData($id); ?>"><?php echo $strandName ?></option>
        <?php
    }

}

function populateSemesterSelectOption(){
    $con=connect();
    $stmt=$con->prepare('SELECT DISTINCT strand_grade FROM strands ORDER BY strand_grade    ');
    $stmt->execute();
    $stmt->store_result();
    $stmt->bind_result($semester);
    while ($row = $stmt->fetch()) {
        ?>
            <option value="<?php echo encryptData($semester); ?>"><?php echo $semester ?></option>
        <?php
    }
}
...
function populateSectionCards() {
    ...
    while ($stmt->fetch()) {
        ...
        <div class="section-card <?php if ($sectionStatus == 1) { echo "section-active"; } else { echo "section-inactive"; } {
            ...
            <h5><?php echo "Semester " . $strandGrade ?></h5>
...
function getCurrentStrandSelectOption($sectionStrandID) {
    ...
    $row = $result->fetch_assoc();
    ?>
    <option class='highlight' value="<?php echo encryptData($row['strand_id']); ?>"><?php echo "[Semester" . $row['strand_grade']. "] " . $row['strand_name']; ?></option>
    <?php
...
<head>
    ...
    <title>Alvas | Manage Sections</title>
</head>
<body>
    <section id="main-UI">
                ...
                <div class="container">
                    <div class="filters-wrapper">
                        ...
                        <div data-aos="fade-down" class="filters-content">
                            ...
                            <a href="?order=strandName">Branch</a>
                        ...
                        <div class="section-card">
                            <div class="container">
                                <form id="add-section-form" method="post">
                                    ...
                                    <select name="create-section-semester">
                                        <option value="">Select Semester</option>
                                        <?php populateSemesterSelectOption()?>
                                    </select>
                                </form>
                            </div>
                        </div>           
